...
import { 
  ArrowRight, Sparkles, Smartphone, Laptop, Headphones, 
  Camera, Watch, Gamepad, Zap, PackageOpen, Monitor, Speaker,
...
const CATEGORY_ICONS: Record<string, any> = {
  'Mobile': Smartphone, 'Phone': Smartphone, 'Laptops': Laptop, 'Computers': Monitor,
  'Audio': Headphones, 'Headphones': Headphones, 'Cameras': Camera, 'Wearables': Watch,
  'Smartwatch': Watch, 'Gaming': Gamepad, 'Accessories': Zap, 'Speakers': Speaker,
};
  ...
  return (
    <div className="space-y-20 pb-20">
        ...
        <div className="relative z-10 px-6 py-16 md:py-24 md:px-16 flex flex-col md:flex-row items-center justify-between gap-12">
           <div className="max-w-2xl space-y-6 text-center md:text-left">
             ...
             <p className="text-lg text-slate-300 leading-relaxed max-w-lg mx-auto md:mx-0">
               Discover the latest gadgets and accessories. Verified quality, fast delivery, and premium support.
             </p>
             <div className="flex flex-col sm:flex-row gap-3 pt-4 justify-center md:justify-start">
               <Link href="/products"><Button size="lg" className="bg-white text-slate-900 hover:bg-slate-100 font-bold px-8 rounded-full w-full sm:w-auto">Shop Now</Button></Link>
               <Link href="/products"><Button size="lg" variant="outline" className="border-slate-700 text-white hover:bg-slate-800 rounded-full w-full sm:w-auto">View Deals</Button></Link>
             </div>
           </div>
           ...
           <div className="hidden md:flex w-full max-w-sm justify-center relative">
              ...
              <div className="relative w-72 h-72 bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl flex items-center justify-center rotate-3 hover:rotate-0 transition-transform duration-500 group cursor-pointer">
                 ...
                 <div className="absolute bottom-6 text-slate-400 font-mono text-xs">FEATURED COLLECTION</div>
              </div>
           </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4">
        <h2 className="text-2xl font-bold tracking-tight text-slate-900 mb-6">Shop by Category</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
          {loading ? [...Array(6)].map((_, i) => <Skeleton key={i} className="h-24 w-full rounded-xl" />) : 
            categories.map((cat, i) => {
              ...
              return (
                <Link key={i} href={`/products?category=${cat}`} className="group">
                  <div className="flex flex-col items-center justify-center p-4 bg-white border border-slate-100 rounded-2xl hover:border-blue-200 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 h-28">
                    ...
                    <span className="font-medium text-sm text-slate-700 group-hover:text-blue-700 text-center line-clamp-1">{cat}</span>
                  </div>
                </Link>
              );
            })
      ...
      <section className="container mx-auto px-4">
        <div className="bg-gradient-to-r from-red-500 to-orange-500 rounded-3xl p-8 md:p-12 text-white relative overflow-hidden">
           ...
           <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="text-center md:text-left space-y-4">
                 ...
                 <h2 className="text-3xl md:text-5xl font-extrabold">Super Sale Ends Soon!</h2>
                 <p className="text-red-100 text-lg max-w-md">Get up to 40% off on selected headphones and gaming gear. Offer valid while stocks last.</p>
                 <div className="flex gap-4 justify-center md:justify-start font-mono text-2xl font-bold">
                    <div className="bg-white/20 p-3 rounded-lg min-w-[60px] text-center">02<div className="text-[10px] font-sans font-normal opacity-70">HRS</div></div>
                    <div className="bg-white/20 p-3 rounded-lg min-w-[60px] text-center">45<div className="text-[10px] font-sans font-normal opacity-70">MIN</div></div>
                    <div className="bg-white/20 p-3 rounded-lg min-w-[60px] text-center">12<div className="text-[10px] font-sans font-normal opacity-70">SEC</div></div>
                 </div>
              </div>
              <div className="shrink-0">
                 <Link href="/products"><Button size="lg" className="bg-white text-red-600 hover:bg-red-50 font-bold px-8 h-12 rounded-full shadow-lg">Shop The Sale</Button></Link>
              </div>
           </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4">
        <div className="flex items-end justify-between mb-8">
          ...
          <div className="hidden sm:flex gap-2">
             <Button variant="outline" size="icon" className="rounded-full" onClick={() => scrollSlider('left')}><ChevronLeft className="h-4 w-4" /></Button>
             <Button variant="outline" size="icon" className="rounded-full" onClick={() => scrollSlider('right')}><ChevronRight className="h-4 w-4" /></Button>
          </div>
        </div>
        ...
        >
          {loading ? [...Array(5)].map((_, i) => (
             <div key={i} className="min-w-[280px] snap-center"><Skeleton className="h-[350px] w-full rounded-xl" /></div>
          )) : products.map((product: any) => (
             <div key={product._id} className="min-w-[260px] md:min-w-[280px] snap-center">
      ...
      <section className="container mx-auto px-4 py-8 border-t border-b">
         <div className="flex flex-wrap justify-center md:justify-between items-center gap-8 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
            ...
            {['SONY', 'SAMSUNG', 'APPLE', 'LOGITECH', 'ASUS', 'DELL'].map((brand, i) => (
               <div key={i} className="text-2xl font-black text-slate-800 tracking-tighter">{brand}</div>
            ))}
         </div>
      </section>
      ...
      <section className="container mx-auto px-4">
        <div className="rounded-3xl bg-slate-100 p-8 md:p-12 text-center md:text-left flex flex-col md:flex-row items-center justify-between gap-8 border border-slate-200">
           <div className="space-y-4 max-w-lg">
              <h2 className="text-3xl font-bold text-slate-900">Newsletter Subscription</h2>
              <p className="text-slate-600">Join our newsletter and unlock exclusive deals, early access to new drops, and special member pricing.</p>
           </div>
           <div className="w-full max-w-md flex gap-2">
              <input type="email" placeholder="Enter your email" className="flex-1 h-12 rounded-full px-5 border border-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-900 transition" />
              <Button size="lg" className="rounded-full bg-slate-900 hover:bg-slate-800">Subscribe</Button>
           </div>
        </div>
      </section>
